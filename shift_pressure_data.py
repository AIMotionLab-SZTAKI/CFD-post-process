...
data_file_name = os.path.join(abs_path, "dynamic_pressure_field_processed_new.csv")
...
np.savetxt("combined_data.txt", combined_1d)

#tmp = np.loadtxt(os.path.join(abs_path, "..", "combined_data.txt"))
#cube_size = int(math.pow(tmp.shape[0] + 1, 1/3))
#combined_data = np.reshape(tmp, (cube_size, cube_size, cube_size))
#print(combined_data.shape)

#plt.imshow(data[:, :, 0], cmap='jet', interpolation='nearest')
plt.imshow(combined_data[:, :, 0], cmap='jet', interpolation='nearest')
plt.show()